...
if flag==1
    g=40;  %input('Size of the problem g, the dimension will be n=2g+1: \n');
...
% this should be avoid since sparsity may be destroyed, see .....
...
Fun_Err=@(i) 2*sum(Ss(i+1:end))+2*(NH-i)*((2/3)*normPin*norm(S_q)*tol+(2/3)*normQin*norm(Z)*tol);
